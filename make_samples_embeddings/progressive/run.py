...
def parse_args():
    ...
    parser.add_argument("--FRACTION", dest="FRACTION",
                      help="{Fraction of samples from same half as test}",
                      default = 10,
                      type=int)
...
class SplitProcessor():
    ...
    def load_file(self, path, random_state = 1, level="L1", fraction= 0, split="train"):
        #1_L1_progressive_0%_train_data.json
        ## Path and data
        self.path = path
        ...
        self.split = split 
        if self.split == "test":
            self.file_name = self.path + f"{self.random_state}_{self.level}_progressive_{self.split}_data.json"
        else:
            self.file_name = self.path + f"{self.random_state}_{self.level}_progressive_{self.fraction}%_{self.split}_data.json"
        self.split_data = pd.read_json(self.file_name, orient="records")
