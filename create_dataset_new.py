...
class HabitatMapsProcessor():
    ...
    def __init__(self, study_area, cantons_list, raw_data_path):
        ...
        self.grid = GridBuilder().grid_from_shape(shape=self.study_area)
...
class SpeciesRecordsProcessor():
    ...
    def trim_species(self, records):
        ...
        records = records[records["speciesKey"].isin(species_list)]
        return records
...
class Step1():
    ...
    def __init__(self):
        ...
        final_data_path = "./WikiSpeciesHabitats/"
        ## Area of interest
...
class Step2():
    ...
    def __init__(self):
        ...
        speciesData = species_records[fields].drop_duplicates()
        speciesData.to_json(final_data_path+"speciesData.json")
...
class Step3():
    ...
    def __init__(self):
        ...
        species_habitats_records.groupby(["zone_id","TypoCH_NUM"])["speciesKey"].agg(["unique"]).reset_index().to_json(final_data_path+"speciesInZones.json", orient="records")
        print("Saved speciesInZones.json, head is the following")
        print(species_habitats_records.groupby(["zone_id","TypoCH_NUM"])["speciesKey"].agg(["unique"]).reset_index().head())
        ## Saving species recorded in each habitat type
        species_habitats_records.groupby(["TypoCH_NUM"])["speciesKey"].agg(["unique"]).reset_index().to_json(processed_data_path+"speciesInHabitats.json", orient="records")
        print("Saved speciesInHabitats.json, head is the following")
        print(species_habitats_records.groupby(["TypoCH_NUM"])["speciesKey"].agg(["unique"]).reset_index())
        ## Saving habitats recorded for each species
        species_habitats_records.groupby(["speciesKey"])["TypoCH_NUM"].agg(["unique"]).reset_index().to_json(processed_data_path+"habitatsOfSpecies.json", orient="records")
        print("Saved habitatsOfSpecies.json, head is the following")
        print(species_habitats_records.groupby(["speciesKey"])["TypoCH_NUM"].agg(["unique"]).reset_index())
        """


class Step4():
    """Step 4 : Getting textual description of species present in the dataset"""
    def __init__(self):
        print("Step 4 : Getting textual description of species present in the dataset")
        ## Paths
        processed_data_path = "./processed_data/"
        final_data_path = "./WikiSpeciesHabitats/"
        ## Dataset records
        records = pd.read_json(processed_data_path+"species_habitats_records.json", orient="records")
