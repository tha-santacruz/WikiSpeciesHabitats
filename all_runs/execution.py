...
class Executer():
    def __init__(self, cfg):
        ...
        self.train_set = WikiSpeciesHabitats(path=self.cfg.DATASET_PATH, split="train", merge_method=self.cfg.MERGE_METHOD, random_state=self.cfg.RANDOM_STATE, 
                                            target_base=self.cfg.TARGET_BASE, splitting=self.cfg.SPLITTING, fraction=self.cfg.FRACTION, embedding=self.cfg.EMBEDDING, level=self.cfg.LEVEL)
        ...
        if self.cfg.LOAD_CHECKPOINT == "yes":
            if self.cfg.SPLITTING == "progressive":
                name = f"/{self.cfg.RANDOM_STATE}_{self.cfg.EMBEDDING}_embedding_{self.cfg.LEVEL}_level_{self.cfg.MERGE_METHOD}_merging_{self.cfg.SPLITTING}_{self.cfg.FRACTION}%_{self.cfg.HIDDEN_SIZE}.pth"
            else:
                name = f"/{self.cfg.RANDOM_STATE}_{self.cfg.EMBEDDING}_embedding_{self.cfg.LEVEL}_level_{self.cfg.MERGE_METHOD}_merging_{self.cfg.SPLITTING}_splitting_{self.cfg.HIDDEN_SIZE}.pth"
